<x-app-layout>
    ...
    <form action="{{ route('strawberi.sell-global.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
        @csrf
        ...
        <div>
            ...
            @error('preferensi_grade')<p class="text-red-600 text-sm">{{ $message }}</p>@enderror
        </div>

        <div>
            <label class="block text-sm font-medium">Jumlah Jual (kg)</label>
            <input type="number" step="0.01" min="0.01" name="jumlah_jual" value="{{ old('jumlah_jual') }}" class="mt-1 w-full border rounded px-3 py-2" required>
            @error('jumlah_jual')<p class="text-red-600 text-sm">{{ $message }}</p>@enderror
            <p class="text-xs text-gray-600 mt-1">Jika stok tidak cukup, sistem akan melakukan penjualan partial otomatis.</p>
        </div>
        ...
        <div>
            ...
            <select id="price_mode" name="price_mode" class="mt-1 w-full border rounded px-3 py-2" required>
                ...
                <option value="buy" {{ old('price_mode')==='buy' ? 'selected' : '' }}>Gunakan harga beli batch (weighted)</option>
            </select>
            @error('price_mode')<p class="text-red-600 text-sm">{{ $message }}</p>@enderror
        </div>

        <div id="harga_jual_group" style="display:none;">
            <label class="block text-sm font-medium">Harga Jual per kg</label>
            <input type="number" step="0.01" min="0" name="harga_jual" value="{{ old('harga_jual') }}" class="mt-1 w-full border rounded px-3 py-2">
            @error('harga_jual')<p class="text-red-600 text-sm">{{ $message }}</p>@enderror
        </div>

        <div>
            <label class="block text-sm font-medium">Pembeli</label>
            <input type="text" name="pembeli" value="{{ old('pembeli') }}" class="mt-1 w-full border rounded px-3 py-2" required>
            @error('pembeli')<p class="text-red-600 text-sm">{{ $message }}</p>@enderror
        </div>

        <div>
            <label class="block text-sm font-medium">Tanggal Jual</label>
            <input type="date" name="tanggal_jual" value="{{ old('tanggal_jual') ?? now()->format('Y-m-d') }}" class="mt-1 w-full border rounded px-3 py-2" required>
            @error('tanggal_jual')<p class="text-red-600 text-sm">{{ $message }}</p>@enderror
        </div>
        ...
        <div>
            ...
            @error('bukti_pembayaran')<p class="text-red-600 text-sm">{{ $message }}</p>@enderror
        </div>
