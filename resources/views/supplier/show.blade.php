        ...
        <div class="flex justify-between items-center">
            ...
            <div class="flex space-x-2">
                <a href="{{ route('supplier.index') }}"
                    class="inline-flex items-center px-4 py-2 bg-gray-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase hover:bg-gray-700 active:bg-gray-800 focus:outline-none focus:border-gray-800 focus:ring ring-gray-300 transition ease-in-out duration-150">
                    ...
                    <div class="p-6 border-b border-gray-200">
                        ...
                        <div class="space-y-4">
                            <div class="bg-gray-50 rounded-lg p-4">
                                <div class="flex justify-between mb-2">
                                    <h4 class="text-sm font-medium text-gray-500">Pembayaran ke suplier (pengeluaran
                                        pembelian stroberi)</h4>
                                    <span class="text-lg font-semibold text-red-600">Rp
                                        {{ number_format($pembayaranKeSupplier, 0, ',', '.') }}</span>
                                </div>
                                <div class="flex justify-between mb-2">
                                    <h4 class="text-sm font-medium text-gray-500">Pinjaman suplier (pengeluaran)</h4>
                                    <span class="text-lg font-semibold text-red-600">Rp
                                        {{ number_format($pinjamanSupplier, 0, ',', '.') }}</span>
                                </div>
                                <div class="flex justify-between mb-2">
                                    <h4 class="text-sm font-medium text-gray-500">Pembayaran dari suplier (pemasukan)
                                    </h4>
                                    <span class="text-lg font-semibold text-green-600">Rp
                                        {{ number_format($pembayaranDariSupplier, 0, ',', '.') }}</span>
                                </div>
                        ...
                        <div class="mt-6 pt-6 border-t border-gray-200">
                            ...
                            <form action="{{ route('supplier.pengembalian', $supplier) }}" method="POST">
                                ...
                                <div class="space-y-4">
                                    <div>
                                        <label for="jumlah_pengembalian"
                                            class="block text-sm font-medium text-gray-700">Jumlah Pengembalian
                                            (Rp)</label>
                                    ...
                                    <div>
                                        <label for="keterangan_pengembalian"
                                            class="block text-sm font-medium text-gray-700">Keterangan
                                            Pengembalian</label>
                                        <div class="mt-1">
                                            <textarea id="keterangan_pengembalian" name="keterangan_pengembalian" rows="2"
                                                {{ $sisaPinjaman <= 0 ? 'disabled' : '' }}
                                                class="shadow-sm focus:ring-green-500 focus:border-green-500 block w-full sm:text-sm border-gray-300 rounded-md {{ $sisaPinjaman <= 0 ? 'bg-gray-100 cursor-not-allowed' : '' }}"
                                                placeholder="Contoh: Pengembalian cicilan ke-1, Transfer BCA, dll"></textarea>
                                        </div>
                                    </div>
                                ...
                                <div class="space-y-4">
                                    <div>
                                        <label for="jumlah_pinjaman"
                                            class="block text-sm font-medium text-gray-700">Jumlah Pinjaman
                                            (Rp)</label>
                                        <div class="mt-1 relative rounded-md shadow-sm">
                                            <div
                                                class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                    ...
                                    <div>
                                        <label for="tanggal_pinjaman"
                                            class="block text-sm font-medium text-gray-700">Tanggal Pinjaman</label>
                                        <div class="mt-1">
                                            <input type="date" name="tanggal_pinjaman" id="tanggal_pinjaman"
                                    ...
                                    <div>
                                        <label for="keterangan_pinjaman"
                                            class="block text-sm font-medium text-gray-700">Keterangan
                                            Pinjaman</label>
                                        <div class="mt-1">
                                            <textarea id="keterangan_pinjaman" name="keterangan_pinjaman" rows="2"
                                                class="shadow-sm focus:ring-red-500 focus:border-red-500 block w-full sm:text-sm border-gray-300 rounded-md"
                                                placeholder="Contoh: Modal usaha, Kebutuhan operasional, dll"></textarea>
                                        </div>
                                    </div>
                    ...
                    <div class="p-6 border-b border-gray-200">
                        ...
                        <div class="space-y-3">
                            <a href="{{ route('supplier.edit', $supplier) }}"
                                class="inline-flex justify-center items-center w-full px-4 py-2 border border-gray-300 shadow-sm text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500">
                            ...
                            <a href="{{ route('strawberi.create') }}?supplier_id={{ $supplier->id }}"
                                class="inline-flex justify-center items-center w-full px-4 py-2 border border-gray-300 shadow-sm text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500">
                    ...
                    <div class="border-b border-gray-200">
                        <nav class="-mb-px flex space-x-8">
                            <button @click="activeTab = 'strawberi'"
                                :class="{ 'border-red-500 text-red-600': activeTab === 'strawberi', 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300': activeTab !== 'strawberi' }"
                                class="py-4 px-1 border-b-2 font-medium text-sm">
                                Strawberi
                            </button>
                            <button @click="activeTab = 'transaksi'"
                                :class="{ 'border-green-500 text-green-600': activeTab === 'transaksi', 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300': activeTab !== 'transaksi' }"
                                class="py-4 px-1 border-b-2 font-medium text-sm">
                                Transaksi
                            </button>
                        </nav>
                                        ...
                                        <tbody class="bg-white divide-y divide-gray-200">
                                            @forelse($transaksis as $transaksi)
                                                <tr>
                                                    ...
                                                    <td
                                                        class="px-6 py-4 text-sm font-medium {{ $transaksi->jenis == 'pemasukan' ? 'text-green-600' : 'text-red-600' }}">
