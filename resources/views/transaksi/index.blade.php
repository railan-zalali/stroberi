                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @forelse($transaksis as $transaksi)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            @if(isset($transaksi->tipe_transaksi))
                                                <span
                                                    class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $transaksi->is_pinjaman ? 'bg-yellow-100 text-yellow-800' : 'bg-blue-100 text-blue-800' }}">
                                                    {{ ucfirst($transaksi->tipe_transaksi) }}
                                                </span>
                                                @if($transaksi->is_pinjaman)
                                                    <span class="ml-1 text-xs text-yellow-600">(Pinjaman)</span>
                                                @endif
                                            @else
